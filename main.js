...
/* ===========================================================
   DÖVİZ KURU ŞERİDİ: CNN Business'ın piyasa veri bandına karşılık
   gelen, bizim alanımıza (dış ticaret) uygun canlı kur şeridi.
   Frankfurter API (ücretsiz, anahtarsız, CORS destekli, ECB
   referans kurları) kullanılıyor. API başarısız olursa şerit
   zarifçe bir hata mesajına düşer, sayfanın geri kalanını
   etkilemez.
   =========================================================== */
(function () {
  ...
  async function loadRates() {
    // Karsilastirma icin ~5 gun onceki bir tarih (hafta sonu/resmi
    // tatil ihtimaline karsi Frankfurter otomatik en yakin is gununu
    // dondurur).
    const past = new Date();
    past.setDate(past.getDate() - 5);
    const pastStr = past.toISOString().slice(0, 10);

    const results = await Promise.all(currencies.map(async (c) => {
      const [curRes, prevRes] = await Promise.all([
        fetch(`https://api.frankfurter.dev/v1/latest?base=${c.code}&symbols=TRY`),
        fetch(`https://api.frankfurter.dev/v1/${pastStr}?base=${c.code}&symbols=TRY`)
      ]);
      if (!curRes.ok || !prevRes.ok) throw new Error('rate fetch failed');
      const cur = await curRes.json();
      const prev = await prevRes.json();
      const curRate = cur.rates.TRY;
      const prevRate = prev.rates.TRY;
      const changePct = ((curRate - prevRate) / prevRate) * 100;
      return { ...c, rate: curRate, changePct };
    }));
...
/* ===========================================================
   HEADER İÇİ KOMPAKT KUR GÖSTERGESİ (sadece anasayfa, #headerRateMini
   elementi varsa çalışır). Aynı Frankfurter API'yi kullanır, ama
   çok daha kompakt bir formatta (sadece kod + değer + ok) gösterir.
   =========================================================== */
(function () {
